Netchain Core version *3.1.1* is now available from:  <https://github.com/netchain-project/netchain/releases>
...
Please report bugs using the issue tracker at github: <https://github.com/netchain-project/netchain/issues>

Non-Mandatory Update
==============

Netchain Core v3.1.1 is a non-mandatory update to address bugs and introduce minor enhancements that do not require a network change.

How to Upgrade
==============

If you are running an older version, shut it down. Wait until it has completely shut down (which might take a few minutes for older versions), then run the installer (on Windows) or just copy over /Applications/Netchain-Qt (on Mac) or netchaind/netchain-qt (on Linux).


Compatibility
==============

Netchain Core is extensively tested on multiple operating systems using
...
Netchain Core should also work on most other Unix-like systems but is not
...
### Fix spending for v1 zNTX created before block 1050020

The transition to v2 zNTX and reset of the accumulators caused blocks 1050000 - 1050010 to be accumulated twice. This was causing a number v1 zNTX to not create valid witnesses, and thus were not spendable. This problem is fixed by double accumulating blocks 1050000-1050010 when creating the witness. Any user that had issues spending zNTX v1 will now be able to convert that into NTX and then zNTX v2 (if desired).

### Adjustment to staking properties to reduce orphaned blocks

zNTX stake set to update more frequently and lowering the stake hashdrift to 30 seconds to reduce the number of orphans being experienced by Netchain stakers.
...
### Show progress percent for zntx reindex operations
...
Netchain Daemon & Client (RPC Changes)
--------------
...
### Core Features
 ...
 - #609 `6b73598b9` [MoveOnly] Remove zNTX code from main.cpp (presstab)
...
### GUI
 ...
 - #600 `217433561` [GUI] Only enable/disable PrivacyDialog zNTX elements if needed. (presstab)
 - #612 `6dd752cb5` [Qt] Show progress percent for zntx reindex operations (Fuzzbawls)
...
### Wallet
 ...
 - #603 `779d8d597` Fix spending for v1 zNTX created before block 1050020. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/netchain-project-translations/).
