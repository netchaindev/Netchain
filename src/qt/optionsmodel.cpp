...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::NTX);
    ...
    if (!settings.contains("nAnonymizeNetchainAmount"))
        settings.setValue("nAnonymizeNetchainAmount", 1000);

    nAnonymizeNetchainAmount = settings.value("nAnonymizeNetchainAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeNetchainAmount"))
        SoftSetArg("-anonymizenetchainamount", settings.value("nAnonymizeNetchainAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in netchain.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNetchainAmount:
            return QVariant(nAnonymizeNetchainAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNetchainAmount:
            nAnonymizeNetchainAmount = value.toInt();
            settings.setValue("nAnonymizeNetchainAmount", nAnonymizeNetchainAmount);
            emit anonymizeNetchainAmountChanged(nAnonymizeNetchainAmount);
            break;
        case CoinControlFeatures:
