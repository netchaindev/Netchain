...
CzNTXWallet::CzNTXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zntx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZNTXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNTXSeed_deprecated()) {
                    LogPrintf("%s: Updated zNTX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zntx seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zntx wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNTXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNTXCount(nCountLastUsed);
    else if (!walletdb.ReadZNTXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNTXWallet::Lock()
{
    seedMaster = 0;
}

void CzNTXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNTXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNTX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNTXWallet::LoadMintPoolFromDB()
{
...
void CzNTXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNTXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNTXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zntxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzNTXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNTX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zntxTracker which also adds to database
    pwalletMain->zntxTracker->Add(dMint, true);
...
void CzNTXWallet::SeedToZNTX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzNTXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNTXWallet::GenerateDeterministicZNTX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNTXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNTXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
