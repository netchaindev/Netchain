...
// The zNTX block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zNTX is a hash of the serial
...
{
    //Create an output returning the zNTX that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZNTXOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zNTX output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zNTX", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZNTXOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zNTX output", __func__);
...
{
    CzNTXTracker* zntxTracker = pwallet->zntxTracker.get();
    CMintMeta meta;
    if (!zntxTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zntxTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a NTX stake is the outpoint
