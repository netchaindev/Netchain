...
public:
    ...
    bool WriteZNTXSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZNTXSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZNTXSeed_deprecated(uint256& seed);
    bool EraseZNTXSeed();
    bool EraseZNTXSeed_deprecated();

    bool WriteZNTXCount(const uint32_t& nCount);
    bool ReadZNTXCount(uint32_t& nCount);
