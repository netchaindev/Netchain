...
#ifndef Netchain_ZNTXWALLET_H
#define Netchain_ZNTXWALLET_H
...
public:
    CzNTXWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZNTX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZNTX(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Netchain_ZNTXWALLET_H
